...
class Trainer:
    ...
    def __init__(
        ...
        # ----------------- start stage semantics (YAML backward compatible) -----------------
        # Old behavior: always start from "warm", then switch by early.patience or pinn.warmup.
        # Fix: if both are <=0, it should mean "no warm stage" (start co-training immediately).

        early_cfg = getattr(cfg, "early", None)
        warm_patience = int(getattr(early_cfg, "patience", 0) or 0)

        # Optional explicit override (won't break old YAML)
        start_stage = getattr(cfg.train, "start_stage", None)
        if start_stage is not None:
            start_stage = str(start_stage).lower().strip()
            if start_stage in ("warm", "warmup", "multi"):
                self.stage = start_stage
            else:
                print(f"[WARN] Unknown train.start_stage={start_stage}, fallback to auto logic.")

        # Auto logic: warmup<=0 and patience<=0 => skip warm, start co-training immediately
        if (getattr(cfg.train, "start_stage", None) is None) and (warm_patience <= 0) and (self.dynamic_warmup <= 0):
            self.stage = "warmup"

        if self.stage != "warm":
            self.pinn_blend = 1.0
            if (self.teacher is not None) and bool(getattr(self.spatial_cfg, "use", False)) and (self.teacher_ref is None):
                self._refresh_teacher_ref(self.model.state_dict())
            if self.stage_events and self.stage_events[0].get("epoch", None) == 0:
                self.stage_events[0] = {"phase": self.stage, "epoch": 0, "global_step": 0}
    ...
    def train_one_epoch(
        ...
        for batch in pbar:
            ...
            phys_info = {"raw": float("nan"), "cap": float("nan"), "scale": 1.0, "triggered": False}
                ...
                if pinn is not None:
                    ...
                    if torch.isfinite(pinn_out["loss_pinn"]):
                        ...
                        if not self.reverse_hierarchy:
                            ...
                            guard_scales.append(phys_guard["scale"])
                        else:
